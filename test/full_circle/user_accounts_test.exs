defmodule FullCircle.UserAccountsTest do
  use FullCircle.DataCase

  alias FullCircle.UserAccounts
  import FullCircle.UserAccountsFixtures
  alias FullCircle.UserAccounts.{User, UserToken}
  ...
  describe "register_user/1" do
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = UserAccounts.register_user(%{email: too_long, password: too_long})
  ...
  describe "change_user_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = UserAccounts.change_user_registration(%User{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        UserAccounts.change_user_registration(
          %User{},
  ...
  describe "apply_user_email/3" do
    ...
    test "requires email to change", %{user: user} do
      {:error, changeset} = UserAccounts.apply_user_email(user, valid_user_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{user: user} do
      {:error, changeset} =
        UserAccounts.apply_user_email(user, valid_user_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{user: user} do
      ...
      {:error, changeset} =
        UserAccounts.apply_user_email(user, valid_user_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        UserAccounts.apply_user_email(user, valid_user_password(), %{email: email})
    ...
    test "validates current password", %{user: user} do
      {:error, changeset} =
        UserAccounts.apply_user_email(user, "invalid", %{email: unique_user_email()})
    ...
    test "applies the email without persisting it", %{user: user} do
      email = unique_user_email()
      {:ok, user} = UserAccounts.apply_user_email(user, valid_user_password(), %{email: email})
      assert user.email == email
      assert UserAccounts.get_user!(user.id).email != email
    end
  end
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          UserAccounts.deliver_update_email_instructions(user, "current@example.com", url)
        end)
  ...
  describe "update_user_email/2" do
    setup do
      ...
      token =
        extract_user_token(fn url ->
          UserAccounts.deliver_update_email_instructions(%{user | email: email}, user.email, url)
        end)
  ...
  describe "delete_session_token/1" do
    test "deletes the token" do
      user = user_fixture()
      token = UserAccounts.generate_user_session_token(user)
      assert UserAccounts.delete_session_token(token) == :ok
      refute UserAccounts.get_user_by_session_token(token)
    end
  end
  ...
  describe "confirm_user/1" do
    ...
    test "confirms the email with a valid token", %{user: user, token: token} do
      assert {:ok, confirmed_user} = UserAccounts.confirm_user(token)
    ...
    test "does not confirm with invalid token", %{user: user} do
      assert UserAccounts.confirm_user("oops") == :error
    ...
    test "does not confirm email if token expired", %{user: user, token: token} do
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert UserAccounts.confirm_user(token) == :error
      refute Repo.get!(User, user.id).confirmed_at
      assert Repo.get_by(UserToken, user_id: user.id)
    end
  end
  ...
  describe "deliver_user_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          UserAccounts.deliver_user_reset_password_instructions(user, url)
        end)
  ...
  describe "get_user_by_reset_password_token/1" do
    ...
    test "returns the user with valid token", %{user: %{id: id}, token: token} do
      assert %User{id: ^id} = UserAccounts.get_user_by_reset_password_token(token)
      assert Repo.get_by(UserToken, user_id: id)
    end

    test "does not return the user with invalid token", %{user: user} do
      refute UserAccounts.get_user_by_reset_password_token("oops")
      assert Repo.get_by(UserToken, user_id: user.id)
    end

    test "does not return the user if token expired", %{user: user, token: token} do
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute UserAccounts.get_user_by_reset_password_token(token)
      assert Repo.get_by(UserToken, user_id: user.id)
    end
  end
  ...
  describe "reset_user_password/2" do
    ...
    test "deletes all tokens for the given user", %{user: user} do
      _ = UserAccounts.generate_user_session_token(user)
      {:ok, _} = UserAccounts.reset_user_password(user, %{password: "new valid password"})
      refute Repo.get_by(UserToken, user_id: user.id)
    end
  end
