defmodule FullCircleWeb.UserSettingsController do
  ...
  def update(conn, %{"action" => "update_email"} = params) do
    ...
    case UserAccounts.apply_user_email(user, password, user_params) do
      {:ok, applied_user} ->
        UserAccounts.deliver_update_email_instructions(
          applied_user,
          user.email,
  ...
  defp assign_email_and_password_changesets(conn, _opts) do
    ...
    conn
    |> assign(:email_changeset, UserAccounts.change_user_email(user))
    |> assign(:password_changeset, UserAccounts.change_user_password(user))
  end
end
